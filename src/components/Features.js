...
import { Files, View, CodeAsTextView } from "../config/consts";
...
export default function Features() {
  ...
  const [viewType, setViewType] = useState(View.Feature);

  const [selectedRow, setSelectedRow] = useState();
  const [featureAsText, setFeatureAsText] = useState();
  ...
  const handleOpenAsText = (featurePath) => {
    console.log(`Selected row content:${JSON.stringify(featurePath)}`);
    ipcRenderer.send("feature:read", JSON.stringify(featurePath));
  };
  ...
  useEffect(() => {
    ...
    ipcRenderer.on("feature:content", (e, featureContent) => {
      console.log(`Feature as Text: ${featureContent}`);
      setFeatureAsText(featureContent);
      setViewType(CodeAsTextView);
    });
  }, []);
  ...
  return (
    <Container>
      ...
      <Row>
        ...
        <Col xs={3}>
          <Button
            variant='danger'
            size='sm'
            onClick={() => handleOpenAsText(selectedRow.featurePath)}
      ...
      {viewType === View.Feature && (
        <ReactTable
          columns={columns}
          data={dataSet}
          onHandleRowSelection={handleRowSelection}
        />
      )}
      {viewType === CodeAsTextView && (
        <TextView code={featureAsText} language={"gherkin"} showLnNum={true} />
      )}
